...
//
//  FC chaining:
//    in_valid pulse -> (3-cycle delay) -> fc1_start pulse -> FC1 runs
//    fc1_done pulse -> (1-cycle delay) -> fc2_start pulse -> FC2 runs
//    fc2_done pulse -> accumulator samples s_fc2 (exactly once per timestep)
// =============================================================================
...
// ── FC1 start: fires 3 cycles after in_valid so conv/pool pipeline settles ────
// Cycle 0 : in_valid=1  -> latch in_reg, start countdown
// Cycle 1,2,3: conv+pool combinational output propagates (registered paths)
// Cycle 3 : fc1_start pulses -> FC1 sees valid s_pool2, begins inference
...
always @(posedge clk) begin
  if (rst) begin
    ...
    if (in_valid) begin
      ...
      if (fc1_start_cnt == 2'd1)
        fc1_start <= 1'b1;     // pulse on last count
    end
  end
end

// ── FC2 start: fires 1 cycle after fc1_done ───────────────────────────────────
// fc1_done is a 1-cycle pulse from ann_layers when all 32 FC1 neurons computed.
// We delay by 1 so s_fc1_raw is stable (registered in ann_layers) before FC2
// reads it.
wire fc1_done;
reg  fc2_start;
...
always @(posedge clk)
    if (fc1_bram_en) fc1_bram_rdata <= fc1_bram[fc1_bram_addr[12:0]];
...
always @(posedge clk)
    if (fc2_bram_en) fc2_bram_rdata <= fc2_bram[fc2_bram_addr[6:0]];
...
always @(posedge clk) begin
    if (rst) begin
        for (ci = 0; ci < FC2_OUT; ci = ci + 1)
            internal_counts[ci] <= 8'd0;
    end else if (fc2_done_pulse_d2) begin   // ← was fc2_done_pulse
        for (ci = 0; ci < FC2_OUT; ci = ci + 1)
            if (s_fc2[ci])
                internal_counts[ci] <= internal_counts[ci] + 8'd1;
    end
end
